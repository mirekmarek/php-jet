...
class Debug_Formatter
{
	...
	public static function formatErrorContext_HTML( array $error_context )
	{
		...
		foreach( self::formatErrorContext( $error_context ) as $var_name => $var_value ) {
			...
			$i++;
			$output[] = '<tr style="'.$row_style.'"><td valign="top"> $'.$var_name.'</td><td>'.static::encodeForHTML(
					$var_value
				).'</td></tr>';
		}
		$output[] = '</table>';
	...
	public static function formatBacktrace( array $debug_backtrace, $expand_call_arguments = true )
	{
		...
		foreach( $debug_backtrace as $d ) {
			...
			$output[] = [
				'file' => $file, 'line' => $line, 'class' => $class, 'type' => $type, 'function' => $function,
				'args' => $args_array, 'call' => $content,
			];
		}
	 ...
	 * @return string
	 */
	protected static function encodeForHTML( $html )
	{

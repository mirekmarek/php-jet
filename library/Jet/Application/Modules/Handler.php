...
/**
 * Class Application_Modules_Handler
 * @package Jet
 */
class Application_Modules_Handler extends Application_Modules_Handler_Abstract {
	...
	public function installModule( $module_name ) {
		...
		if( !$module_manifest->getIsCompatible() ) {
			throw new Application_Modules_Exception(
				'Module \''.$module_name.'\' (API version '.$module_manifest->getAPIVersion().') is not compatible with this system version (API version'.Version::getAPIVersionNumber().')',
	...
	protected function _readModulesList( $ignore_corrupted_modules, $base_dir, $module_name_prefix ) {
		...
		foreach( $modules as $module_dir ) {
			if( !IO_File::exists( $base_dir.$module_dir.'/'.static::MODULE_MANIFEST_FILE_PATH ) ) {
				...
				$this->_readModulesList($ignore_corrupted_modules, $base_dir.$module_dir.'/', $next_module_name_prefix);
				continue;
			}
	...
	protected function _saveInstalledModulesList() {
		...
		IO_File::write($this->modules_list_file_path, '<?php'.JET_EOL.' return '.var_export($this->installed_modules_list, true).';'.JET_EOL);
	}

...
class Entity_Validator_Definition extends BaseObject
{
	protected string|false $type = '';
	
	protected bool $is_sub_validator = false;
	protected bool $is_sub_validators = false;
	...
	/**
	 * @var array<string,Entity_Validator_Definition_ValidatorOption>
	 */
	...
	protected function init( object $context_object, string $property_name, array $definition_data ) : void
	{
		...
		if($this->is_sub_validators || $this->is_sub_validator) {
			return;
		}
		 ...
		 * @var Validator $class
		 */
		$option_definition = $class::getValidatorOptionsDefinition();
		
		foreach($this->other_options as $option=>$value) {
			if(!isset($option_definition[$option])) {
				throw new Entity_Validator_Definition_Exception('Validator definition '.get_class($context_object).'::'.$property_name.' - unknown option \''.$option.'\'');
			}
		...
		if(!$this->type) {
			throw new Entity_Validator_Definition_Exception('Validator definition '.get_class($context_object).'::'.$property_name.' - field type is not specified');
		}

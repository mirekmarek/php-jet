 ...
 *
 */
class DataModel_Backend_MSSQL extends DataModel_Backend
{
	...
	/**
	 * @var ?DataModel_Backend_MSSQL_Config
	 */
	...
	{
		return DataModel_Backend::TYPE_MSSQL;
	}
	...
	public function prepareDefaultDbConnectionConfig() : ?Db_Backend_Config
	{
		...
		$db_connection_config->setDriver( Db::DRIVER_MSSQL );
	...
	public function helper_getCreateCommand( DataModel_Definition_Model $definition, ?string $force_table_name = null ): string
	{
		...
		foreach( $definition->getKeys() as $key_name => $key ) {
			...
			switch( $key->getType() ) {
				case DataModel::KEY_TYPE_PRIMARY:
					...
					$keys[$key_name] = PHP_EOL . "\t" . ',PRIMARY KEY (' . $key_columns . ')';
					break;
				case DataModel::KEY_TYPE_INDEX:
					$keys[$key_name] = PHP_EOL . "\t" . ',INDEX ' . $this->_quoteName( $key_name ) . ' (' . $key_columns . ')';
					break;
				default:
					$keys[$key_name] = PHP_EOL . "\t" . ',' . $key->getType() . ' INDEX ' . $this->_quoteName(
							$key_name
						) . ' (' . $key_columns . ')';
					break;
			}
		...
		$table_name = $force_table_name ? : $this->_getTableName( $definition, false );
		
		$create_index_query = implode( PHP_EOL, $create_index_query );
		
		$q = 'IF NOT EXISTS (SELECT * FROM sysobjects WHERE name=\''.$table_name.'\' and xtype=\'U\') '.PHP_EOL;
		$q .= ' CREATE TABLE [' . $table_name . '] (' . PHP_EOL;
		$q .= implode( ',' . PHP_EOL, $_columns );
		$q .= implode( '', $keys );
		$q .= PHP_EOL . ') ' . $_options . ';' . PHP_EOL;
		$q .= PHP_EOL.PHP_EOL;
	...
	{
		$column_name = strtolower(substr($property_definition->getDatabaseColumnName(),  0, 128));
		...
		$table_name = $this->_getTableName( $property_definition->getDataModelDefinition(), $quote );
	...
	{
		$this->getDbWrite()->getConfig()->getDbname();
		$table_name = $model_definition->getDatabaseTableName();
		$table_name = substr( $table_name, 0, 128);
	...
	{
		$backend_options = $column->getBackendOptions( DataModel_Backend::TYPE_MSSQL );
		...
		switch( $column->getType() ) {
			...
			case DataModel::TYPE_STRING:
				...
				if( $column->getIsId() ) {
					return 'nvarchar(' . ($max_len) . ') NOT NULL  DEFAULT \'\'';
				} else {
					if($max_len>4000) {
						return 'ntext';
					} else {
						return 'nvarchar(' . ($max_len) . ') DEFAULT ' . $this->_getValue( $default_value );
					}
				}
	...
	public function helper_getDropCommand( DataModel_Definition_Model $definition ): string
	{
		...
		return 'EXEC sp_rename ' . $this->_quoteName( $table_name ) . ', ' . $this->_quoteName( $ui_prefix . $table_name ) . '';
	}
	...
	public function helper_getUpdateCommand( DataModel_Definition_Model $definition ): array
	{
		...
		$exists_cols = $this->getDbWrite()->fetchCol( "SELECT name FROM sys.columns WHERE object_id = OBJECT_ID('$table_name') " );
		
		$update_prefix = '_UP' . date( 'YmdHis' ) . '_';
		$updated_table_name = $update_prefix . $this->_getTableName( $definition, false );
		$backup_table_name = $update_prefix . 'b_' . $this->_getTableName( $definition, false );
		...
		$rename_command1 = 'EXEC sp_rename [' . $table_name . '], [' . $backup_table_name . '];';
		$rename_command2 = 'EXEC sp_rename [' . $updated_table_name . '],  [' . $table_name . '];';
	...
	public function createCountQuery( DataModel_Query $query ): string
	{
		...
		return 'SELECT count(DISTINCT CHECKSUM('.$id_properties.')) FROM ' . PHP_EOL
	...
	protected function _getSqlQueryLimitPart( DataModel_Query $query ): string
	{
		...
		if( $limit ) {
			$limit_qp = PHP_EOL . ' OFFSET ' . $offset . ' ROWS FETCH NEXT '.$limit.' ROWS ONLY' . PHP_EOL;
		}

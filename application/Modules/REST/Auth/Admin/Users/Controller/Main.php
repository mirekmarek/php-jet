...
class Controller_Main extends MVC_Controller_Default
{
		...
		if( !$this->router ) {
			$this->router = new MVC_Controller_Router_AddEditDelete(
				...
				item_catcher: function( $id ) : bool {
					return (bool)($this->user = APIUser::get( (int)$id ));
				},
				actions_map: [
			...
			$this->router->addAction( 'reset_password', Main::ACTION_UPDATE )
				->setResolver( function() {
					return (
						...
						($this->user = APIUser::get( Http_Request::GET()->getInt( 'id' ) ))
					);
				} )
	...
	public function add_Action(): void
	{
		...
		if( $user->catchAddForm() ) {
			$password = APIUser::generatePassword();
			$user->setPassword( $password );
			$user->save();

			Logger::success(
				event: 'rest_user_created',

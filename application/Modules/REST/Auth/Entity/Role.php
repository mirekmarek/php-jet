...
class Role extends DataModel implements Auth_Role_Interface
{
	...
	#[DataModel_Definition(
		...
		data_model_class: Role_Privilege::class,
	)]
	...
	{
		return APIUser_Roles::getRoleUsers( $this->id );
	}
	...
	{
		APIUser_Roles::roleDeleted($this->id);
	}
	...
	public function setPrivilege( string $privilege, array $values ): void
	{
		if($values) {
			if( !isset( $this->privileges[$privilege] ) ) {
				$this->privileges[$privilege] = new Role_Privilege( $privilege, $values );
			} else {
	...
	public static function getAclActionValuesList_ModulesActions( bool $translate = true, ?Locale $translate_locale=null ): Data_Forest
	{
		...
		foreach( $modules as $module_name => $module_info ) {
			...
			if(!str_contains($module_name, 'REST')) {
				continue;
			}

...
class CompatibilityTester
{
	...
	{
		foreach( [
			         'function_exists', 'class_exists', 'version_compare', 'ini_get', 'ob_start', 'ob_end_clean',
			         'ob_get_contents', 'phpinfo',
	...
	{
		$this->test(
			'PHP version', 'PHP 5.5.4 or newer is required', function( CompatibilityTester_TestResult $test_result ) {
	...
	{
		$this->test(
			'PDO extension', 'PHP PDO extension must be activated', function() {
	...
	{
		$this->check(
			'INTL extension', 'PHP Internationalization Functions extension must be activated',
			function( CompatibilityTester_TestResult $test_result ) {
				$result = extension_loaded( 'intl' );

				if( !$result ) {
					$test_result->setResultMessage(
						'<b style="color:red">Internationalization support is limited!</b>'
					);
				}
	...
	{
		$this->test(
			'$_SERVER["REQUEST_URI"] value', 'PHP $_SERVER["REQUEST_URI"] value must be available', function() {
	...
	{
		if( $this->check(
			'GD extension', 'PHP GD extension should be activated', function() {
			...
			$this->check(
				...
				function( CompatibilityTester_TestResult $test_result ) {
					/** @noinspection SpellCheckingInspection */
					$functions = [
						'imagecreatefromjpeg', 'imagecreatefromgif', 'imagecreatefrompng', 'imagecolorallocatealpha',
						'imagefilledrectangle', 'imagecopyresampled', 'imagejpeg', 'imagegif', 'imagepng',
					];
	...
	{
		$this->check(
			'FileInfo extension', 'PHP FileInfo extension should be activated', function() {
	...
	{
		$this->check(
			'PHP configuration: Max upload file size', '', function( CompatibilityTester_TestResult $test_result ) {
			...
			if( $upload_max_filesize<2097152 ) {
				$test_result->setResultMessage(
					'<i>upload_max_filesize</i> ('.$upload_max_filesize_cv.') should be greater then 2MB'
				);
	...
	{
		if( $this->check(
			'PHP configurations - paths', 'open_basedir, upload_tmp_dir and session.save_path',
			function( CompatibilityTester_TestResult $test_result ) {
			...
			$this->test(
				'open_basedir vs upload_tmp_dir', '', function( CompatibilityTester_TestResult $test_result ) {
				...
				if( !$result ) {
					$test_result->setResultMessage(
						'upload_tmp_dir is '.$upload_tmp_dir.' but open_basedir is '.$open_base_dirs
					);
				}
			...
			$this->test(
				'open_basedir vs session.save_path', '', function( CompatibilityTester_TestResult $test_result ) {
				$open_base_dirs = explode( ':', ini_get( 'open_basedir' ) );
				...
				if( !$result ) {
					$test_result->setResultMessage(
						'session.save_path is '.$session_save_path.' but open_basedir is '.$open_base_dirs
					);
				}

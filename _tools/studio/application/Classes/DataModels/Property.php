...
abstract class DataModels_Property extends DataModel_Definition_Property implements DataModels_Property_Interface {
	...
	/**
	 * @param DataModel_Definition_Property $jet_property
	 *
	 * @return DataModels_Property
	 */
	public static function createFromJetProperty( DataModel_Definition_Property $jet_property )
	{
		$class_name = explode('_', get_class( $jet_property ));
		$class_name = $class_name[count($class_name)-1];
		$class_name = __NAMESPACE__.'\\DataModels_Property_'.$class_name;
		...
		foreach( get_object_vars($jet_property) as $k=>$v ) {
			if(
				$k=='data_model_class_name' ||
				$v===null
			) {
				continue;
			}
			...
			if(
				$k=='form_field_get_select_options_callback' &&
				is_array($v)
			) {
				$v = '[\''.$v[0].'\',\''.$v[1].'\']';
			}
	...
	public static function getCreateForm()
	{
		if(!static::$create_form) {
			...
			$property_name->setValidator( function( Form_Field_Input $field ) {
				return DataModels_Property::checkPropertyName( $field );
			} );
	...
	public static function catchCreateForm()
	{
		...
		$new_property->setInternalPriority(999999);

		DataModels::getCurrentModel()->addProperty( $new_property );
		DataModels::getCurrentModel()->checkSortOfProperties();

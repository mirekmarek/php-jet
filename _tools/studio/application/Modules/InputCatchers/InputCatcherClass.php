...
class InputCatcherClass extends ClassMetaInfo
{
	
	/**
	 * @return InputCatcherClass_Property[]
	 */
	public function getProperties() : array
	{
		...
		if($i instanceof Entity_InputCatcher_Interface) {
			$input_catcher_definition = $i->getPropertyInputCatchersDefinition();
		} else {
			$input_catcher_definition = [];
		}
		...
		foreach($this->reflection->getProperties() as $property) {
			...
			$p = new InputCatcherClass_Property( $this, $property->getName(), $property, $input_catcher_definition[$property->getName()]??null );
	...
	{
		return InputCatcherClass::get( $class_name );
	}
}
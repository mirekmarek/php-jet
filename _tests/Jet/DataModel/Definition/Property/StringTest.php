...
class DataModel_Definition_Property_StringTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testSetUpRelation()
	{
		...
		$this->id_object->setUpRelation(
			$this->id_model_related->getDataModelClassName(), $this->id_model_related->getName()
		);
	...
	{
		$this->assertEquals(
			'Jet\DataModel_Definition_DataModelTestMock::'.$this->property_name, (string)$this->object
		);
	}
	...
	{
		$this->assertEquals(
			$this->property_options['form_field_is_required'], $this->object->getFormFieldIsRequired()
		);
		$this->assertEquals(
			$this->id_property_options['form_field_is_required'], $this->id_object->getFormFieldIsRequired()
		);
	}
	...
	{
		$this->assertEquals(
			$this->property_options['backend_options']['test'], $this->object->getBackendOptions( 'test' )
		);
	}
	...
	public function testGetFormFieldOptions()
	{
		...
		$this->assertEquals(
			$this->property_options['form_field_options'], $this->object->getFormFieldOptions()
		);
	}
	...
	{
		$this->assertEquals(
			$this->property_options['form_field_label'], $this->object->getFormFieldLabel()
		);
	}
	...
	{
		$this->assertEquals(
			$this->property_options['form_field_error_messages'], $this->object->getFormFieldErrorMessages()
		);
	}
	...
	public function testGetFormField()
	{
		...
		$field->__test_set_state(
			[
				...
				'_value'                 => $this->property_options['default_value'], '_has_value' => false,
				'is_valid'               => false, 'last_error' => '', 'last_error_message' => '',
				'default_value'          => $this->property_options['default_value'],
				'label'                  => $this->property_options['form_field_label'], 'is_required' => true,
				'validation_regexp'      => $this->property_options['form_field_validation_regexp'],
				'validate_data_callback' => null, 'error_messages' => [
		...
		$field->__test_set_state(
			[
				...
				'_value'            => $this->id_property_options['default_value'], '_has_value' => false,
				'is_valid'          => false, 'last_error' => '', 'last_error_message' => '',
				'default_value'     => $this->id_property_options['default_value'], 'label' => '',
				'is_required'       => false, 'validation_regexp' => null, 'validate_data_callback' => null,
				'select_options'    => [],
			]
		);
	...
	{
		$this->assertEquals(
			$this->property_options['form_field_validation_regexp'], $this->object->getFormFieldValidationRegexp()
		);
	}
	...
	protected function setUp()
	{
		...
		$this->id_object = new $class_name(
			get_class( $this->data_model ), $this->id_property_name, $this->id_property_options
		);
		$this->id_model_related = new $class_name(
			get_class( $this->data_model ), 'id_related', $this->id_property_options
		);
	}

...
class DataModel_Query_OrderByTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testIterator()
	{
		...
		$this->assertEquals(
			[
				'+int_property', '+my_value', '-string_property', '+id_property',
			], $data
		);
	}
	...
	protected function setUp()
	{
		...
		$this->query->setSelect(
			[
				'this.string_property', 'my_value' => [
				[ 'this.int_property' ], 'MAX(%int_property%)',
			],
			]
		);

		$this->object = new DataModel_Query_OrderBy(
			$this->query, [
				            '+this.int_property', '+my_value', '-string_property', 'this.id_property',
			            ]
		);
	}

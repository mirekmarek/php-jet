...
class Http_HeadersTest extends \PHPUnit_Framework_TestCase
{
	...
	protected $test_headers = [
		'Test-header'       => 'test_value', 'Test-header-wo-value',
	...
	protected function getValidDataTestResponse( $code, $add_redirect_target = false )
	{
		...
		$res[] = [
			'header'             => 'HTTP/1.1 '.$code.' '.$this->http_codes[$code], 'replace' => true,
			'http_response_code' => $code,
		];

		if( $add_redirect_target ) {
			$res[] = [
				'header' => 'Location: '.$this->redirect_target, 'replace' => true, 'http_response_code' => 0,
			];
		}

		foreach( $headers as $h ) {
			$res[] = [
				'header' => $h, 'replace' => true, 'http_response_code' => 0,
			];
	...
	public function testMovedPermanently()
	{
		...
		$this->assertEquals(
			$this->getValidDataTestResponse( 301, true ), $GLOBALS['_test_Http_Headers_sent_headers']
		);
	}
	...
	public function testMovedTemporary()
	{
		...
		$this->assertEquals(
			$this->getValidDataTestResponse( 302, true ), $GLOBALS['_test_Http_Headers_sent_headers']
		);
	}
	...
	public function testSeeOther()
	{
		...
		$this->assertEquals(
			$this->getValidDataTestResponse( 303, true ), $GLOBALS['_test_Http_Headers_sent_headers']
		);
	}
	...
	protected function setUp()
	{
		...
		Http_Headers::setHeaderFunctionName( __NAMESPACE__.'\header_test' );
		$GLOBALS['_test_Http_Headers_sent_headers'] = [];
	}

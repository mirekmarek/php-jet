...
class IO_DirTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testCopy()
	{
		...
		$this->assertEquals(
			'IO_Dir::copy test', file_get_contents( $this->copy_test_target_path.'subdir/subdir/file.txt' )
		);
	}
	...
	public function testGetList()
	{
		...
		$this->assertEquals( $this->getValidDataGetListTest(), IO_Dir::getList( IO_DIR_TEST_BASEDIR.'getlist' ) );
		$this->assertEquals(
			$this->getValidDataGetListTest( true, true, true ), IO_Dir::getList( IO_DIR_TEST_BASEDIR.'getlist/', '*N*' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( true, true, true ), IO_Dir::getList( IO_DIR_TEST_BASEDIR.'getlist', '*N*' )
		);
	...
	{
		$files_list = [
			'SubDir1', 'SubDir2', 'SubDirN', 'file1.txt', 'file2.txt', 'fileN.txt',
		];
	...
	{
		$this->assertEquals(
			$this->getValidDataGetListTest( false ), IO_Dir::getFilesList( IO_DIR_TEST_BASEDIR.'getlist/' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( false ), IO_Dir::getFilesList( IO_DIR_TEST_BASEDIR.'getlist' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( false, true, true ),
			IO_Dir::getFilesList( IO_DIR_TEST_BASEDIR.'getlist/', '*N*' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( false, true, true ),
			IO_Dir::getFilesList( IO_DIR_TEST_BASEDIR.'getlist', '*N*' )
	...
	{
		$this->assertEquals(
			$this->getValidDataGetListTest( true, false ),
			IO_Dir::getSubdirectoriesList( IO_DIR_TEST_BASEDIR.'getlist/' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( true, false ),
			IO_Dir::getSubdirectoriesList( IO_DIR_TEST_BASEDIR.'getlist' )
		...
		$this->assertEquals(
			$this->getValidDataGetListTest( true, false, true ),
			IO_Dir::getSubdirectoriesList( IO_DIR_TEST_BASEDIR.'getlist/', '*N*' )
		);
		$this->assertEquals(
			$this->getValidDataGetListTest( true, false, true ),
			IO_Dir::getSubdirectoriesList( IO_DIR_TEST_BASEDIR.'getlist', '*N*' )

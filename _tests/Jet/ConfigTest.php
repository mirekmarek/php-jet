...
class ConfigTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testGetAvailableHandlersList()
	{
		...
		$this->assertEquals(
			$valid_handlers_list, ConfigTestMock::getAvailableHandlersList( '_data/Config/testGetHandlersList' )
		);
	}
	...
	public function testGetCommonForm()
	{
		...
		$valid_form = new Form(
			'Jet_ConfigTestDescendantMock', [
				                              new Form_Field_Input(
					                              'string_property', 'String property:', 'String config value', true
				                              ), new Form_Field_Int( 'int_property', 'Int property:', 123, false ),
				                              new Form_Field_Float( 'float_property', 'Float property:', 1.3, true ),
				                              new Form_Field_Checkbox(
					                              'bool_property', 'Bool property:', false, false
				                              ), new Form_Field_Input(
					                              'next_string_property', 'Next string property:',
					                              'Next string config value', true
				                              ),
			                              ]
		);

		$valid_form->getField( 'string_property' )->setErrorMessages(
			[ Form_Field_Input::ERROR_CODE_EMPTY => 'string is empty' ]
		);
		$valid_form->getField( 'next_string_property' )->setErrorMessages(
			[ Form_Field_Input::ERROR_CODE_EMPTY => 'string is empty' ]
		);
		$valid_form->getField( 'float_property' )->setErrorMessages(
			[ Form_Field_Input::ERROR_CODE_EMPTY => 'float is empty' ]
		);
	...
	public function testCatchFormAndSave()
	{
		...
		$data = [
			'string_property' => 'String config value - updated', 'float_property' => 1.4, 'int_property' => 123456789,
			'bool_property'   => true, 'next_string_property' => 'Next string config value - updated',
		];
		...
		$this->assertEquals(
			[
				'section' => [
					'subsection' => [
						'string_property' => 'String config value - updated', 'int_property' => 123456789,
						'float_property' => 1.4, 'bool_property' => true,
	...
	public function testToArray()
	{
		...
		$this->assertEquals(
			[
				'string_property' => 'String config value', 'int_property' => 123, 'float_property' => 1.3,
				'bool_property'   => false, 'next_string_property' => 'Next string config value',
			], $this->object->toArray()
		);
	}
